...
### H3: `act`'s Host Network or Artefact-Server Options Trigger the Container Failure
...
artefact server setup, make container startup fail.
...
| 3    | Run a minimal temporary workflow through `act` that has no upload-artefact step and only echoes text.                        | The minimal workflow fails identically, showing artefact-server options are not necessary. |
...
### H3: Host Network or Artefact-Server Options Trigger the Failure
...
by `act`, not the workflow, runner image, host networking, or artefact server.
